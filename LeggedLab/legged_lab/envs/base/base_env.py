...
from legged_lab.mdp import Gait
import torch
...
class BaseEnv(VecEnv):
    def __init__(self, cfg: BaseEnvCfg, hedless):
        ...
        self.gait_generator = Gait(cfg=cfg.gait, env=self)
        ...
        self.feet_pos_f = torch.zeros(self.num_envs, len(self.feet_cfg.body_ids), 3, device=self.device)
        self.feet_pos_h = torch.zeros(self.num_envs, len(self.feet_cfg.body_ids), 3, device=self.device)
    ...
    def init_buffers(self):
        ...
        self.terrain_height_scan = torch.zeros(self.num_envs, 1, dtype=torch.float, device=self.device, requires_grad=False)
    ...
    def compute_current_observations(self):
        ...
        gait = self.gait_generator.gaits
        first_foot_swing_height_target = self.gait_generator.first_foot_swing_height_target
        second_foot_swing_height_target = self.gait_generator.second_foot_swing_height_target
        gait_indices = self.gait_generator.gait_indices
        swing_height_indices = self.gait_generator.swing_height_indices
        ...
        current_actor_obs = torch.cat([
            ...
            gait,
            first_foot_swing_height_target.view(self.num_envs, 1),
            second_foot_swing_height_target.view(self.num_envs, 1),
            gait_indices.view(self.num_envs, 1),
            swing_height_indices.view(self.num_envs, 1),
    ...
    def reset(self, env_ids):
        ...
        self.gait_generator.resample_gaits(env_ids)
        self.gait_generator.gait_indices[env_ids] = 0
    ...
    def post_step_callback(self):
        ...
        # env_ids = (self.episode_length_buf % int(self.cfg.gait.resampling_time / self.step_dt) == 0).nonzero(as_tuple=False).flatten()
        # self.gait_generator.resample_gaits(env_ids)
        self.gait_generator.step_contact_targets()
        ...
        if self.cfg.scene.height_scanner.enable_height_scan:
            self.terrain_height_scan = self.height_scanner.data.ray_hits_w[..., 2].mean(dim=-1).unsqueeze(1)

        self.compute_feet_pos()

    def compute_feet_pos(self):
        foot_ids = [11, 12]
        self.feet_pos_f = self.robot.data.body_pos_w[:, foot_ids, :]
        self.feet_pos_f[:, :, 2] += -0.048 - self.terrain_height_scan
        self.feet_pos_f[:, :, 2] = torch.clip(self.feet_pos_f[:, :, 2], 0, 1)

        feet_quat = self.robot.data.body_quat_w[:, foot_ids, :]
        delta_feet_pos = torch.zeros_like(self.feet_pos_f)
        delta_feet_pos[..., 0] -= 0.05519231
        delta_feet_pos[:, 0] = math_utils.quat_rotate(feet_quat[:, 0], delta_feet_pos[:, 0])
        delta_feet_pos[:, 1] = math_utils.quat_rotate(feet_quat[:, 1], delta_feet_pos[:, 1])
        self.feet_pos_h = self.feet_pos_f + delta_feet_pos
        self.feet_pos_h[:, :, 2] = torch.clip(self.feet_pos_h[:, :, 2], 0, 1)
